...
- **Bots run on the host.** Empty lobby seats are filled with bot players (`{ bot: true }`, no peer id) when the host starts; `buildSeating` in `useGame.js` spreads humans apart (two humans → seats 0 & 2, bots on the sides). After every `broadcast()`, the host's `runBots` driver checks whether the seat that must act next (a discard turn, or a pending call) is a bot and, if so, schedules its move (`BOT_DELAY`) — applying it re-broadcasts and chains the next bot move. `broadcast()` skips bots (no peer to send a view to). A human who disconnects mid-game is converted in place to a bot (`onPeerLeave`), so the round never stalls. Bot decisions live in `bot.js` and only read the bot's own hand + public board (no cheating). They pon/chi to advance, but `botCallResponse` only takes a call that **keeps a yaku reachable** (a yakuhai triplet, or an all-simples tanyao shape) since an open hand can't win yakuless — except looser bots, which call even into a dead hand. A host lobby **bot difficulty** choice (`botDifficulty`: `'easy'`/`'normal'`/`'hard'`, default `'normal'`) is threaded into `botTurnAction`/`chooseDiscard`/`botCallResponse`: `'hard'` is the flawless, deterministic baseline (also the function default, which the tests rely on — it always takes a sound call and never misdiscards), while `'easy'`/`'normal'` add a per-move `mistakeChance` (shed a random tile) and call less soundly (`callChance`, `looseCalls`, `maxCallShanten`). All levels still always riichi when offered and always take a win. Difficulty stays host-side (bots only run on the host), so it needs no network plumbing.
...
- **Match length** is a host lobby choice → `createGame(players, { maxRoundWind })`: `'1z'` East-only (tonpuusen, default), `'2z'` East+South (hanchan, also accepted as `{ hanchan: true }`), `'4z'` all four winds. `nextRound` ends the match when a full lap of `maxRoundWind` completes (keeping that round's wind/number and result so the game-over dialog shows), or on a bankruptcy.
- **Turn clock** is a host lobby choice (`TIME_LIMITS` in `useGame.js`: `off` default, `5+20`, `60`, `300` — `base` free seconds each move plus a `bank` reserve). It lives entirely in the host hook, **not** the engine (timing isn't a rule; the engine stays pure). MahjongSoul-style: each move refills only `base`; the reserve is a **per-seat pool** (`bankLeftRef`) seeded once at `startGame` that drains (never refills) for the whole match. After each `broadcast()`, `updateClock` opens a window for each present human who must act (deadline = `base` + that seat's remaining reserve), charges a seat's reserve as its window closes (`settleSeat`, time used beyond `base`), arms one `setTimeout` for the earliest deadline, and rides a per-seat `{ [seat]: { remaining, bank, token } }` map along on every view. Clients tick locally (`TurnTimer` in `Pieces.jsx`, keyed by `token`), showing base draining first then the reserve. On expiry `onTimeout` plays a safe default for the seats whose own deadline passed — tsumogiri the drawn tile, or pass the call. Bots are excluded (the bot driver paces them).
